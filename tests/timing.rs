...
impl Metadata<Math> for Meta {
    ...
    fn modify(eclass: &mut EClass<Math, Self>) {
        ...
        if best.children.is_empty() {
            eclass.add_node(ENode::leaf(best.op.clone()));
        }
    }
}
...
fn time_egg() {
	...
	if(batch.len() > 0) {
		...
		Some(StopReason::NodeLimit(lim)) => (),
		Some(StopReason::Saturated) => {println!("saturated")},
		Some(StopReason::TimeLimit(t)) => {println!("time limit exceeded")},
